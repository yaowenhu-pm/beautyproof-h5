...
export function parseLinkPage(html, platform, workUrl, finalUrl, httpStatus = 200) {
    ...
    if (platform === 'douyin' && finalUrl.hostname === 'jingxuan.douyin.com') {
        ...
        if (httpStatus >= 400)
            return empty('network_error');
        if (!id || !/^\/m\/video\/\d+\/?$/.test(finalUrl.pathname))
            return empty('unsupported_page');
        let value = parseState(html, '_SSR_DATA');
        for (const key of ['data', 'storeState', 'detail', 'videoData', 'result'])
            value = rec(value)?.[key];
        const summary = rec(value);
        if (!summary || typeof summary.gid !== 'string' || !summary.gid)
            return empty('parse_failed');
        if (summary.gid !== id || ['aweme_id', 'awemeId'].some(key => summary[key] != null && summary[key] !== id))
            return empty('identity_mismatch');
        if (typeof summary.abstract !== 'string' || !summary.abstract.trim())
            return empty('parse_failed');
