export const KB_VERSION = '2026-09-06.1';
...
export const evidence: Evidence[] = [
  ...
  { id: 'CN-AD11', title: '中华人民共和国广告法', section: '第11、17条', version: '2021修正；2026-09-06核阅', url: 'https://www.samr.gov.cn/zw/zfxxgk/fdzdgknr/fgs/art/2023/art_5474cf75173c45d6a0379730fb4e8d97.html', text: '广告中使用数据、统计、文摘、引用语等，应真实准确并注明出处。医疗、药品、医疗器械以外的广告不得涉及疾病治疗。仅见同款、名人姓名或外国地名不能证明虚假背书；应核对确切引述及来源。', terms: ['同款','日本','艺妓','IKKO','晚晚','权威','临床','%','百分','数据','专家','治疗'] },
...
export function findIngredients(text: string) {
  ...
  for(const item of ingredients)for(const alias of [item.cn,item.inci,...item.aliases]){
    if(alias==='水')continue;
    const start=lower.indexOf(alias.toLowerCase()); if(start<0)continue;
    if(/^[a-z0-9 ]+$/i.test(alias)&&(/[a-z]/i.test(text[start-1]??'')||/[a-z]/i.test(text[start+alias.length]??'')))continue;
    matches.push({start,end:start+alias.length,item,quote:text.slice(start,start+alias.length)});
  }
